...
const changePin = async (req, res, next) => {
    try {
        ...
        if (!isValidPin(currentPin) || !isValidPin(newPin)) {
            return next(createHttpError(400, "Both PINs must contain 4 to 6 digits."));
        }
        const user = await User.findById(req.user._id).select("+pin");
        if (!user || !(await bcrypt.compare(currentPin, user.pin))) {
            return next(createHttpError(401, "Current PIN is incorrect."));
        }
...
module.exports = { getPinStatus, setupPin, login, changePin, getUserData, logout, kitchenAuth };
