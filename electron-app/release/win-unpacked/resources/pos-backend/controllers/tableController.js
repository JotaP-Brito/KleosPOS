...
const updateTable = async (req, res, next) => {
  try {
    ...
    if (orderId !== undefined) updates.currentOrder = orderId;
    if (tableNo !== undefined) updates.tableNo = tableNo;
    if (seats !== undefined) updates.seats = seats;
    const table = await Table.findByIdAndUpdate(id, updates, { new: true, runValidators: true });
...
const deleteTable = async (req, res, next) => {
  try {
    ...
    if (table.currentOrder || table.status === "Occupied") return next(createHttpError(400, "Não é possível remover uma mesa com pedido ativo."));
